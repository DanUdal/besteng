...
void Shader::render()
{
  ...
  glUseProgram(id); pollForError(); //runs the shader program

  int activeTexture = 0;
  int vertices = -1;

  for(std::vector<std::shared_ptr<VariableInfo> >::iterator it = cache.begin();
    it != cache.end(); it++) //this loop enters all the uniform variables into the shader from the array of variables
  {
    if((*it)->attrib == false) //this checks if a variable is an attribute or a uniform
    {
      if((*it)->type == GL_FLOAT_MAT4) //tests for each data type for the uniform to use the correct function
      {
    ...
    else //if the variable is an attribute
    {
      int size = 0;
  ...
  if(vertices == -1)
  {
    throw Exception("No vertices were submitted for drawing"); //this runs if there were no attribute variables
  }

  glDrawArrays(GL_TRIANGLES, 0, vertices); pollForError(); //draws the screen

  glUseProgram(0); pollForError(); //turns off the program
